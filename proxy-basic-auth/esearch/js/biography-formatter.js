...
import { uncleanString } from './clean-string.js';
...
function getPersonEntryHTML(entry, sr) {
  ...
  const html = `<a  class="details-button float-end" onclick="showDetails(${entry.referent_db_id})"
                    title="Show source document and details for ${entry.all_name}">Details</a>` +
                (sr.user_is_authenticated
                  ? `<a class="details-button float-end" style="margin-right: 1em;" 
